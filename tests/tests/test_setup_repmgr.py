...
def test_setup_repmgr_packages_debian():
    ...
    hosts = [get_primary(), get_witness()[0], get_standbys()[0]]
    pg_version = get_pg_version()
    packages = ["postgresql-%s-repmgr" % pg_version]
    for host in hosts:
        for package in packages:
            assert host.package(package).is_installed, (
                "Package %s not installed" % packages
...
def test_setup_repmgr_user():
    ...
    for host in hosts:
        ...
        assert len(result) > 0, "repmgr was not succesfully created"
...
def test_setup_repmgr_node_status_redhat():
    ...
    for host in hosts:
        with host.sudo(pg_user):
            cmd = host.run(
                "/usr/pgsql-%s/bin/repmgr -f /etc/repmgr/%s/repmgr-main.conf node status"
                % (pg_version, pg_version)
            )
            result = cmd.stdout.strip()

        assert len(result) > 0, "repmgr command node status failed"
...
def test_setup_repmgr_node_check_redhat():
    ...
    for host in hosts:
        with host.sudo(pg_user):
            cmd = host.run(
                "/usr/pgsql-%s/bin/repmgr -f /etc/repmgr/%s/repmgr-main.conf node check"
                % (pg_version, pg_version)
            )
            result = cmd.stdout.strip().split("\n")

        assert len(result) > 0, "repmgr command node check failed"
