...
def test_setup_dbt2_packages():
    host = get_primary()
    packages = ["dbt2-db", "dbt2-pgsql-c_14", "dbt2-pgsql-plpgsql", "dbt2-scripts"]
...
def test_setup_dbt2_user():
    ...
    with host.sudo(pg_user):
        ...
        cmd = host.run(
            'psql -p %s -At -h %s -c "%s" postgres' % (pg_port, socket_dir, query)
        )
        result = cmd.stdout.strip()

    assert len(result) > 0, "%s was not sucessfully created." % postgres_user
