import pytest
from conftest import get_pemserver, get_pg_type, get_pg_unix_socket_dir, get_pg_version
...
def test_setup_pemserver_pemagent():
    ...
    assert len(result) > 0, "pemagent user was not succesfully created"
...
def test_setup_pemserver_pemadmin():
    ...
    assert len(result) > 0, "pemadmin user was not succesfully created"
...
def test_setup_pemserver_service():
    ...
    assert host.service(service).is_running, "Pemagent service not running"

    assert host.service(service).is_enabled, "Pemagent service not enabled"
...
def test_setup_pemserver_creation():
    ...
    with host.sudo(pg_user):
        query = r"\list"
        cmd = host.run(
            'psql -At -h %s -c "%s" postgres | grep pem' % (socket_dir, query)
        )
        result = cmd.stdout.strip()

    assert len(result) > 0, "PEM database was not succesfully created"
...
def test_setup_pemserver_web_interface():
    ...
    result = result.split("\n")
    assert "200" in result[len(result) - 1], "PEM web interface is not up and running."
